...
- PathSum: `reduce()` (and the `x` / `cx` FFI wrappers) canonicalize the path-variable gauge: row echelon over `out_state`, bare parities for internal variables, a canonical `v` vs `1⊕v` choice, and a signature-based relabeling. Commuting `H`s in either order, `SX` vs `H S H`, nam `h` vs ibm `RZ(π/2) SX RZ(π/2)`, `H X` vs `Z H`, `H0 H1 CX01` vs `CX10 H0 H1`, and X/Y-axis Pauli sandwiches with different conjugator encodings intern equal. Costs about 2.8× on the SX-heavy `pathsum_benchmarking` workload (up to 24 live path variables); H-free states pay one early return.
