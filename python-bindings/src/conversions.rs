...
convert_struct!(
    ...
    egglog::ast::GenericRule<String, String>: "{:?}" => Rule(
        ...
        r -> egglog::ast::GenericRule {span: r.span.clone().into(), head: egglog::ast::GenericActions(r.head.iter().map(|v| v.into()).collect()), body: r.body.iter().map(|v| v.into()).collect(), name: (&r.name).into(), ruleset: (&r.ruleset).into(), naive: false, no_decomp: false},
