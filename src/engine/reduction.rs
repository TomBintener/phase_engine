    ...
    (
        ...
        impl EvaluatedPathSum {
            ...
            pub fn canonicalize_gauge(&mut self) -> bool {
                ...
                let nonlinear = |t: &$primitive| (*t & path_mask) != 0 && !is_zero_or_pow2(*t);
                ...
                let pick = |fresh: $primitive, phase_poly: &CanonicalPhasePoly| -> $primitive {
                    if is_zero_or_pow2(fresh) { return fresh; }
                ...
                for (parity, &ticks) in self.continuous_poly.parities.iter().zip(self.continuous_poly.phases.iter()) {
                    ...
                    if pv == 0 { continue; }
                    if !is_zero_or_pow2(pv) { parity_adj |= pv; }
                    if is_zero_or_pow2(*parity) { bare[pv.trailing_zeros() as usize] = ticks as u64; }
                    let mut f = pv;
                    while f != 0 {
                ...
                if tied != 0 {
                    ...
                    for t in self.phase_poly.terms.iter() {
                        ...
                        if is_zero_or_pow2(tv) { continue; }
                        let mut f = tv;
                        while f != 0 {
            ...
            fn constant_gauge_wants_flip(&self, v: $primitive) -> bool {
                ...
                for t in self.phase_poly.terms.iter() {
                    ...
                    let delta = if is_zero_or_pow2(m) {
                    ...
                    if original_num_path_vars >= 2 {
                        ...
                        fn scan_structure(
                            ...
                            for &t in terms {
                                ...
                                let linear = pv == t && is_zero_or_pow2(pv);
                                while pv != 0 {
